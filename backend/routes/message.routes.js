...
// GET /api/messages/conversations — list all people I've chatted with
...
// GET /api/messages/:partnerId — get chat with one person
router.get('/:partnerId', protect, async (req, res) => {
  try {
    ...
    await Message.update({ isRead: true }, { where: { senderId: req.params.partnerId, receiverId: req.user.id, isRead: false } });
    res.json(msgs);
  } catch (err) { res.status(500).json({ message: err.message }); }
});
...
router.post('/', protect, async (req, res) => {
  try {
    ...
    const msg = await Message.create({ senderId: req.user.id, receiverId, text, shopId: shopId||null });
