...
router.post('/', protect, async (req, res) => {
  try {
    ...
    const order = await PreOrder.create({ shopId, buyerId: req.user.id, items, pickupTime, locationNote });
    res.status(201).json(order);
  } catch (err) { res.status(500).json({ message: err.message }); }
});

// GET /api/preorders/mine  — buyer view
...
router.put('/:id/status', protect, async (req, res) => {
  try {
    const order = await PreOrder.findByPk(req.params.id, { include: [{ model: Shop, as: 'shop' }] });
