...
import { Plus, Eye, Edit, Trash2, BarChart2, ArrowLeft, TrendingUp, MessageCircle } from 'lucide-react';
import api from '../utils/api';
import { useAuth } from '../context/AuthContext';
import { CAT_ICONS, photoUrl } from '../utils/helpers';

function AnalyticsPanel({ shopId }: { shopId: string }) {
  const [data, setData]     = useState<any>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    api.get(`/analytics/${shopId}`).then(({data})=>setData(data)).catch(()=>setData(null)).finally(()=>setLoading(false));
  }, [shopId]);

  if (loading) return <div className="h-32 bg-gray-50 rounded-xl animate-pulse"/>;
  if (!data)   return <div className="text-xs text-cav-text-muted p-4">Analytics unavailable</div>;

  const maxViews = Math.max(...(data.weekly?.map((w:any)=>parseInt(w.count))||[1]), 1);
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-3 gap-3">
        {[
          ...
          <div key={i} className="bg-cav-green-accent/10 rounded-xl p-3 text-center">
            <div className="text-cav-green mb-1 flex justify-center">{m.icon}</div>
            <div className="font-display font-bold text-xl text-cav-green-dark">{m.val}</div>
            <div className="text-xs text-cav-text-muted">{m.label}</div>
          </div>
        ))}
      </div>
      ...
      {data.weekly?.length > 0 && (
        <div>
          <div className="text-xs font-bold text-cav-green-dark mb-2">Views — Last 7 Days</div>
          <div className="flex items-end gap-1 h-20">
            {data.weekly.map((w:any, i:number) => (
              <div key={i} className="flex-1 flex flex-col items-center gap-1">
                <div className="w-full bg-cav-green rounded-t-sm transition-all"
                  style={{height:`${(parseInt(w.count)/maxViews)*100}%`, minHeight:4}}/>
                <div className="text-[9px] text-cav-text-muted">{new Date(w.date).toLocaleDateString('en',{weekday:'short'})}</div>
              </div>
            ))}
          </div>
        </div>
    ...
    <div className="page-container">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <h1 className="font-display font-bold text-2xl text-cav-green-dark">My Shops</h1>
        </div>
        ...
        <div className="space-y-4">
          {shops.map(shop => {
            ...
            const avgRating = shop.reviews?.length
              ? (shop.reviews.reduce((a:number,r:any)=>a+r.stars,0)/shop.reviews.length).toFixed(1) : null;
