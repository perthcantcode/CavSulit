...
export function Messages() {
  ...
  useEffect(() => {
    if (!user) return;
    loadConversations();
    // If opened from shop page, auto-open that conversation
  ...
  const openConversation = async (partner: any, shopId = '') => {
  ...
  const sendPreorder = async () => {
    ...
    try {
      ...
      setMessages(m => [...m, data]);
      setShowPreorder(false); setPreItem(''); setPreTime(''); setPreLoc('');
  ...
  if (!user) return (
    <div className="page-container text-center py-20">
      ...
      <h2 className="font-display font-bold text-xl text-cav-green-dark mb-2">Login to view messages</h2>
      ...
      <div className="flex h-[calc(100vh-64px)] overflow-hidden">
        ...
        <div className={`${activePartner?'hidden md:flex':'flex'} flex-col w-full md:w-72 border-r border-gray-100 bg-white`}>
          <div className="p-4 border-b border-gray-100">
            <h2 className="font-display font-bold text-lg text-cav-green-dark">Messages</h2>
          </div>
        ...
        {/* Chat panel */}
        <div className={`${activePartner?'flex':'hidden md:flex'} flex-1 flex-col bg-cav-bg`}>
          {!activePartner ? (
                  ...
                  return (
                    <div key={i} className={`flex ${isMe?'justify-end':'justify-start'}`}>
                      <div className={`max-w-[75%] px-4 py-2.5 rounded-2xl text-sm leading-relaxed whitespace-pre-wrap
                        ${isMe ? 'bg-cav-green text-white rounded-br-sm' : 'bg-white text-cav-text-dark rounded-bl-sm shadow-sm border border-gray-100'}`}>
                        {m.text}
                      </div>
                    </div>
                  );
