...
import { MapPin, Eye, MessageCircle, Heart, Star, ShoppingBag, Calendar, ArrowLeft, Send } from 'lucide-react';
...
export function ShopDetails() {
  ...
  const [tab,     setTab]     = useState<'info'|'reviews'|'preorder'>('info');
  ...
  const submitReview = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setShop((s:any) => ({ ...s, reviews: [data, ...(s.reviews||[])] }));
      setReviewComment(''); setTab('reviews');
  ...
  const imgs  = shop.photos?.map((p:string) => photoUrl(p)).filter(Boolean);
      ...
      <div className="grid md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-2 space-y-4">
          ...
          <div className="flex gap-1 bg-gray-100 rounded-xl p-1">
            {(['info','reviews','preorder'] as const).map(t=>(
