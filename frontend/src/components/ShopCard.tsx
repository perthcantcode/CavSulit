...
interface ShopCardProps { shop: any; saved?: boolean; onSaveToggle?: (id:string, saved:boolean) => void; }

export function ShopCard({ shop, saved = false, onSaveToggle }: ShopCardProps) {
      ...
      <div className="p-3">
        ...
        {shop.products?.length > 0 && (
          <div className="text-xs font-bold text-cav-green mb-1">
            From ₱{Math.min(...shop.products.map((p:any)=>parseFloat(p.price)))}
          </div>
        )}
